...
# SECURE API KEY SETUP: Reads from OS Environment Variables (No hardcoded keys!)
...
async def run_debate_lifecycle(topic: str, agent_a: str, agent_b: str, total_rounds: int) -> AsyncGenerator[str, None]:
    ...
    # Using the fast stable Gemini flash model to prevent any Round 3/4 lag
    try:
    ...
    # Send start event to initialize UI monitors
    yield "data: " + json.dumps({
        "event": "start", 
        "topic": topic,
        "agent_a": agent_a,
            ...
            for chunk in response:
                try:
                    if chunk.text:
                        ...
                        await asyncio.sleep(0.005) # Maintain smooth elegant flow speed
                except Exception:
                    continue
